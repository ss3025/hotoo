> [来源](http://www.herongyang.com/Year_zh/index.html)

----

> 本书开始部份简单地介绍了日历的基本原理，同时也介绍了目前中国使用的公历和 农历的基本推算规则。中间部份列出了一个中国农历推算原程式。这个程式采用了 压缩数据，可以用来推算二百年农历日期。本书的其余部份收录了由这个程式制作 的二百年年历。
